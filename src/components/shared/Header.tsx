      ...
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-4 px-4 sm:h-[72px] sm:px-6 lg:px-8">
        ...
        >
          {navLinks.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className="text-sm font-medium text-neutral transition hover:text-brand"
            >
              {link.label}
            </a>
          ))}
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <button
            type="button"
            aria-label="Cuenta"
            className="flex size-10 items-center justify-center rounded-full border border-brand/15 text-brand transition hover:bg-surface"
          >
